...
class Build {
  ...
  constructor (options) {
  	...
  	this.buildDir = path.join(this.modpackDir, this.options.version);
...
DirectoryIndex index.json`;
  ...
  setupModIndex() {
    var modpack = {
      ...
      recommended: this.options.version,
      latest: this.options.version,
      builds: [
        // Todo: add multiple
    ...
    return this.writeData(path.join(this.modpackDir, 'index.json'), modpack);
  }
  ...
  setupVersionIndex() {
    ...
    var lockfile = require('../lazuli.lock.json');
  ...
  setupModpackList() {
    var modpackListFile = path.join(this.modpackListDir, 'index.json');
    ...
    .then((fileExists) => {
      ...
      modpackList.modpacks[this.options.slug] = {
        name: this.options.slug,
        url: this.options.url || '',
        display_name: this.options.name,
        recommended: this.options.version,
  ...
  copyMods(destdir, eachdir) {
    ...
    return new Promise((resolve, reject) => {
      ...
      let promises = [];
      let lockfile = require('../lazuli.lock.json');
      for(let modname in lockfile.mods) {
