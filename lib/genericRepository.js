...
class GenericRepository {
  ...
  getMod(match) {
    var newFile = this.generateModFilename(match.name, match.version);
  ...
  async remove(name, force) {
    var matchName = path.join(this.modsdir,
    ...
    return aGlob(matchName).then((fileMatches) => {
      if(fileMatches && Array.isArray(fileMatches) && fileMatches.length > 0) {
        ...
        fileMatches.forEach(match => fs.unlink(match));
      }
      else {
  ...
  updateLockfile(mod) {
    ...
    var filename = path.join(process.cwd(), 'lazuli.lock.json');
